...
#     num_train_epochs: int = 0.5,  # use only 0.05 epoch for training
...
#     benchmark_result = benchmark(llm, testset, 500)
...
def custom_data_collator(features):
    ...
    def pad(t, pad_val):
        return torch.cat([t, torch.full((max_length - t.shape[0],), pad_val, dtype=t.dtype)])

    return {
        "input_ids": torch.stack([pad(f["input_ids"], processor.tokenizer.eos_token_id) for f in features]),
...
class VQADatasetForTraining(Dataset):
    def __init__(self, dataset, processor):
        self.dataset = dataset
        self.processor = processor
        self.processor.tokenizer.pad_token = processor.tokenizer.eos_token
    ...
    def __getitem__(self, idx):
        ...
        prompt = [{"role": "user", "content": [{"type": "image"}, {"type": "text", "text": item["question"]}]}]
        input_text = self.processor.apply_chat_template(prompt, add_generation_prompt=True) + item["answer"]

        inputs = self.processor(images=image, text=input_text, return_tensors="pt", padding=True, truncation=True)
        input_ids = inputs["input_ids"].squeeze(0)
        attention_mask = inputs["attention_mask"].squeeze(0)
        pixel_values = inputs["pixel_values"].squeeze(0)

        answer_ids = self.processor(text=item["answer"], return_tensors="pt", truncation=True).input_ids.squeeze(0)
        answer_len = len(answer_ids)
...
def train(
    ...
    vlm = BaseVLM()
    model = get_peft_model(vlm.model, LoraConfig(
        task_type=TaskType.CAUSAL_LM,
        inference_mode=False,
        r=lora_r,
        lora_alpha=lora_alpha,
    ...
    dataset = VQADatasetForTraining(VQADataset(train_dataset_name, data_dir), processor)
