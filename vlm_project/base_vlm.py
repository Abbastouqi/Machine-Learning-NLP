...
class BaseVLM:
    ...
    def format_prompt(self, question: str) -> str:
     if "left or right" in question.lower():
         hint = "Answer with 'left' or 'right'."
     elif "front or behind" in question.lower():
         hint = "Answer with 'front' or 'behind'."
     elif "how many" in question.lower():
        hint = "Answer with a number."
     elif "track" in question.lower():
        hint = "Answer with the name of the track, e.g., 'black_forest'."
     else:
        hint = ""
...
def test_benchmark():
    model = BaseVLM()
    dataset = VQADataset("valid_grader")
